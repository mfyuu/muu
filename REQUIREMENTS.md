...
- 選択して Enter:
  - タスクに引数が定義されていない場合 → 即座にコマンドを実行
  - タスクに引数が定義されている場合 → 各引数を `inquire::Text` でプロンプト表示し、入力後にコマンドを実行
    - 任意引数（デフォルト値あり）: デフォルト値をプロンプトに表示し、Enter で採用
    - 必須引数（デフォルト = `""`）: 空欄のまま Enter → バリデーションエラー（再入力を促す）
