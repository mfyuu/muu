...
- あいまい検索・フィルタ対応（`inquire` クレートの `Select` が提供する組み込みフィルタリングを利用。自前実装は不要）
- 選択結果は **stdout** に出力、セレクター UI は **stderr** に描画（inquire のデフォルト動作）
- 選択して Enter → `runz <選択したタスク> `（末尾にスペース）を **stdout** に出力して終了
- ESC でキャンセル → 何も出力せず終了コード 1 で終了
- シェルの入力バッファへの配置は `runz shell-init` が生成するシェル関数経由で実現する（後述）
...
### `runz shell-init <shell>`

シェル連携用の関数を stdout に出力する。ユーザーは `.zshrc` に以下の1行を追加してセットアップする:

```bash
eval "$(runz shell-init zsh)"
```

- `zsh` が指定された場合、`runz` シェル関数を定義するスクリプトを出力する（同名のバイナリをシャドウし、内部で `command runz` によりバイナリを呼び出す）
- `runz` 関数はセレクターを起動し、選択結果を `print -z` でシェル入力バッファに配置する
- サポートしないシェルが指定された場合はエラー
...
| サポートしないシェル | エラー: `error: unsupported shell '<名前>'` |
...
- `runz shell-init zsh` は同名の `runz` シェル関数を定義するスクリプトを出力する（バイナリをシャドウ）
- セットアップ: `.zshrc` に `eval "$(runz shell-init zsh)"` を追加
- `runz` シェル関数の動作:
  1. `runz` のセレクターを起動（stderr はターミナルにリダイレクト）
  2. 選択結果（stdout）を `print -z` でシェル入力バッファに配置
  3. ユーザーは引数を追加してから Enter で実行できる
  4. ESC キャンセル時は何もしない
...
- CLI パーサー: `clap`
- インタラクティブセレクター: `inquire`（組み込みファジーフィルタリング付き `Select`）
